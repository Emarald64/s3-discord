...
In the s3s folder create an export folder and a results folder inside of that. Copy config.toml.template to config.toml and set results_dir to the results folder your created.

Create a Discord bot (I don't remember how, look it up) and put the bot's token in discord_token in the config.toml

Invite the bot to a server (look it up). Turn on developer mode in discord settings->Advanced. Copy the channel id that you want the bot to send the messages to and put in updates_channel_id in config.toml
